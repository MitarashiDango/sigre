...
func TestParseSignatureParameters(t *testing.T) {
	...
	testCases := []struct {
		...
		expected      *sigre.ExportForTesting_signaturesParameters // 期待されるパース結果
		...
		// 正常系テストケース
		{
			...
			expected: &sigre.ExportForTesting_signaturesParameters{
				KeyId:     "test-key-1",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			actual, err := sigre.ExportForTesting_parseSignatureParameters(tc.input)
